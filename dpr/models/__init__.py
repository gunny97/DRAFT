...
def init_hf_simcse_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_simcse_biencoder_components

    return get_simcse_biencoder_components(args, **kwargs)


def init_hf_simcse_reader(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_simcse_reader_components

    return get_simcse_reader_components(args, **kwargs)
...
def init_hf_simcse_tenzorizer(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_simcse_tensorizer
    return get_simcse_tensorizer(args.do_lower_case, args.encoder.sequence_length)
