...
class DenseIndexer(object):
    ...
    def search_with_radius(self, query_vectors:np.array, radius:int):
        raise NotImplementedError
...
class DenseFlatIndexer(DenseIndexer):
    ...
    def search_with_radius(self, query_vectors:np.array, radius:float):

        query_top_idxs = []
        for query in query_vectors:
            lims, D, I = self.index.range_search(query.reshape(1,-1), float(radius))
            query_top_idxs.append(list(I))

        query_top_idxs = sum(query_top_idxs,[])
        # convert to external ids
        db_ids = [self.index_id_to_db_id[i] for i in query_top_idxs]
        
        result = [db_ids[i] for i in range(len(db_ids))]
