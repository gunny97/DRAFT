...
class ArgsBase():
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--lang',
                type=str,
                default="kor",
                help='enter language type kor/eng')
...
def load_dataset(path, batch_size, tokenizer, lang):
    ...
    if lang == 'kor':
        ...
        trainset = train_dataset.sample(frac=1).reset_index(drop=True)
        validset = valid_dataset.sample(frac=1).reset_index(drop=True)
...
def evaluate(ckpt_path, device, valid_dataloader):
    # device = torch.device(f"cuda:{device_num}") if torch.cuda.is_available() else torch.device("cpu")
    model = AutoModelForSequenceClassification.from_pretrained(ckpt_path, num_labels=9, ignore_mismatched_sizes=True)
...
def main(ckpt_path, data_path, batch_size, num_epochs, device, finetuned_ckpt_path, lang):
    tokenizer = AutoTokenizer.from_pretrained(ckpt_path)
    model = AutoModelForSequenceClassification.from_pretrained(ckpt_path, num_labels=9, ignore_mismatched_sizes=True)
    torch.cuda.empty_cache()

    train_dataloader, valid_dataloader = load_dataset(data_path, batch_size, tokenizer, lang)
