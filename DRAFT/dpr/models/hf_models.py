...
def get_simcse_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    question_encoder = HFSimCSEEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
    ...
    ctx_encoder = HFSimCSEEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
    ...
    print('Load Question Encoder & Passage Encoder done!!!!!')
...
def get_simcse_reader_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    encoder = HFSimCSEEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
...
def get_simcse_tensorizer(do_lower_case: bool, sequence_length: int):
    tokenizer = get_simcse_tokenizer(do_lower_case=do_lower_case)
    return SimCSETensorizer(tokenizer, sequence_length)
...
def get_simcse_tokenizer(do_lower_case: bool = True):
    # still uses HF code for tokenizer since they are the same
    return AutoTokenizer.from_pretrained("princeton-nlp/sup-simcse-bert-base-uncased", do_lower_case=do_lower_case)
...
class HFSimCSEEncoder(BertModel):
    # def __init__(self, config, project_dim: int = 0):
    def __init__(self, config, project_dim: int = 0):
        BertModel.__init__(self, config)
        assert config.hidden_size > 0, "Encoder hidden_size can't be zero"
    ...
    def init_encoder(
        cls, cfg_name: str, projection_dim: int = 0, dropout: float = 0.1, pretrained: bool = True, **kwargs
        # cls, projection_dim: int = 0, dropout: float = 0.1, pretrained: bool = True, **kwargs
    ):
        logger.info("Initializing HF SimCSE Encoder. cfg_name=%s", cfg_name)
        cfg = AutoModel.from_pretrained("princeton-nlp/sup-simcse-bert-base-uncased").config
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
        ...
        if pretrained:
            ...
            return HFSimCSEEncoder(cfg, project_dim=projection_dim)
    ...
    def forward(
        ...
        elif self.model.config.output_hidden_states:
            sequence_output, pooled_output, hidden_states = out
        else:
            hidden_states = None
            out = super().forward(
...
class SimCSETensorizer(Tensorizer):
    def __init__(self, max_length: int, pad_to_max: bool = True):
        self.tokenizer = AutoTokenizer.from_pretrained("princeton-nlp/sup-simcse-bert-base-uncased", do_lower_case=do_lower_case)
        self.max_length = max_length
        self.pad_to_max = pad_to_max
...
# class SimCSETensorizer(SimCSETensorizer):
#     def __init__(self, tokenizer, max_length: int, pad_to_max: bool = True):
#         super(SimCSETensorizer, self).__init__(tokenizer, max_length, pad_to_max=pad_to_max)